...
class GeneratorSource(DataSource):
    ...
    @staticmethod
    def from_command(main: str, *args: str) -> "GeneratorSource":
        if main.endswith(".py"):
            args = ("python3", main) + tuple(args) # type: ignore
            main = "/usr/bin/env"
        # Run with --sha256 to generate just a SHA256 hash of the output
        output = subprocess.run((main,) + tuple(args) + ("--sha256",),
                            capture_output=True)
        out = output.stdout.decode() + output.stderr.decode()
        if output.returncode != 0:
            raise ValueError(f"Command ({main} {" ".join(args)}) failed: {out}")
        sha = out.strip()
        if len(sha) != len(hashlib.sha256(b"").hexdigest()):
            raise ValueError(f"Invalid SHA256 hash: {sha}")
        return GeneratorSource(main, args, sha)
    ...
    def prepare(self, repo: DataRepository | None = None) -> Data:
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            logger.info(f"Executing: {self.main} {' '.join(self.args)} --output-dir {str(tempdir)}")
            res = subprocess.run(
                (self.main,) + tuple(self.args) + ("--output-dir", str(tempdir)),
